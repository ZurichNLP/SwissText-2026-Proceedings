...
% For Vietnamese characters, with T1 as the default encoding for Latin extended characters.
...
\begin{titlepage}
  ...
  \begin{center}
    ...
    {\bf\LARGE Proceedings of the 11th Edition of the}\\
    {\bf\LARGE Swiss Text Analytics Conference}

    \vfill

    {\LARGE \VAR{conference_dates}, \VAR{conference.start_date.year}}\\[0.35cm]
    {\LARGE University of Zurich}\\[0.35cm]
    {\LARGE \VAR{conference.location}}
  \end{center}
\end{titlepage}
\newpage
...
\begin{minipage}[c]{0.48\linewidth}
  \includegraphics[width=0.85\linewidth]{\VAR{root}/sponsor_logos/SwissNLP-Logo-subline_70percentsize.png}
\end{minipage}
\hfill
\begin{minipage}[c]{0.48\linewidth}
  \raggedleft
  \includegraphics[width=0.75\linewidth]{\VAR{root}/sponsor_logos/university-of-zurich.jpeg}
\end{minipage}
...
% Fixed spacing between committee blocks
\newcommand{\CommitteeBlockSpace}{\vspace{3mm}}

% Role title style
\newcommand{\CommitteeRole}[1]{%
  \noindent\textbf{#1:}\par
}

% One member per line
\newcommand{\CommitteeMemberLine}[1]{%
  \noindent #1\par
}

% Program committee paragraph
\newcommand{\ProgramCommitteeParagraph}[1]{%
  \noindent #1\par
}

\BLOCK{for role in organizing_committee}

  \BLOCK{if role.role is equalto("Invited Speakers")}

    %%%%%%%%%%%%%%%%%%%%%
    % Program Committee %
    %%%%%%%%%%%%%%%%%%%%%
    \BLOCK{if program_committee}
      \CommitteeRole{Program Committee}

      \BLOCK{for block in program_committee}
        \BLOCK{if block.entries[0].area}
          \BLOCK{for area in block.entries}
            \noindent\emph{\VAR{area.area}}\par
            \BLOCK{if area.members[0].institution}
              \BLOCK{for member in area.members}
                \CommitteeMemberLine{\VAR{committee_member(member)}}
              \BLOCK{endfor}
            \BLOCK{endif}
            \BLOCK{if not area.members[0].institution}
              \ProgramCommitteeParagraph{\VAR{join_name(", ", area.members)}}
            \BLOCK{endif}
          \BLOCK{endfor}
        \BLOCK{elif block.type is equalto("name_block")}
          \ProgramCommitteeParagraph{\VAR{join_names(", ", block.entries)}}
        \BLOCK{elif block.type is equalto("name_block_without_newlines")}
          \ProgramCommitteeParagraph{\VAR{to_string_sorting_by_last_name(block.entries)}}
        \BLOCK{else}
          \noindent
          \BLOCK{for member in block.entries}
            \VAR{committee_member(member)}\BLOCK{if not loop.last}, \BLOCK{endif}
          \BLOCK{endfor}
          \par
        \BLOCK{endif}
      \BLOCK{endfor}
...
\BLOCK{for talk in invited_talks}
  ...
  \begin{center}
    ...
    %\begin{minipage}[c][0.35\linewidth][c]{0.35\linewidth}
     % \end{minipage}\\
    \BLOCK{if talk.speaker_name}
    \textbf{\VAR{talk.speaker_name}}\\
    \BLOCK{elif talk.speaker}
    \textbf{\VAR{talk.speaker.first_name} \VAR{talk.speaker.last_name}}\\
    \BLOCK{endif}
    \BLOCK{if talk.institution}
    \VAR{talk.institution}\\
    \BLOCK{elif talk.speaker and talk.speaker.institution}
    \VAR{talk.speaker.institution}\\
    \BLOCK{endif}
...
\begin{itemize}[leftmargin=*,label={}]
  \BLOCK{set ns = namespace(track=None, chapter=0)}
  \BLOCK{for paper in archival_papers}
     \BLOCK{set track=""}
     \BLOCK{if paper.attributes is defined and paper.attributes.submitted_area is defined}
     \BLOCK{set track=paper.attributes.submitted_area}
     \BLOCK{endif}
     \BLOCK{if track and track != ns.track}
       \BLOCK{set ns.chapter = ns.chapter + 1}
       \item \textbf{\VAR{ns.chapter} \VAR{track}}\dotfill \hyperlink{page.\VAR{paper.start_page - 1}}{\VAR{paper.start_page - 1}}
       \BLOCK{set ns.track = track}
     \BLOCK{endif}
...
\BLOCK{for paper in archival_papers}
  ...
  \BLOCK{if track and track != ns.track}
    ...
    \thispagestyle{plain}
    \noindent
    {\Large\bf Chapter \VAR{ns.chapter}}\\[1em]
    {\LARGE\bf \VAR{track}}
    \newpage
    \BLOCK{set ns.track = track}
  \BLOCK{endif}
  \AddToShipoutPicture*{
    ...
    \BLOCK{if paper.watermark_title is defined and paper.watermark_title}
    \BLOCK{set title=paper.watermark_title}
    \BLOCK{elif conference.watermark_book_title}
