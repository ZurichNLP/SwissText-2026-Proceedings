...
def generate_proceedings(path: str, overwrite: bool, outdir: str, nopax: bool, frontmatter: bool):
    ...
    base_title = conference.get("watermark_book_title") or conference["book_title"]
    for paper in archival_papers:
        track = paper.get("track")
        if track:
            paper["watermark_title"] = f"{base_title} -- {track}"
        else:
            paper["watermark_title"] = base_title
...
def process_papers(papers, root: Path):
    ...
    for paper in papers:
        ...
        track = None
        if "attributes" in paper and isinstance(paper["attributes"], dict):
            track = paper["attributes"].get("submitted_area")
        paper["track"] = track
        ...
        paper["start_page"] = page + offset
        paper["end_page"] = page + offset + pdf.getNumPages() - 1
        ...
        for author in paper["authors"]:
            ...
            author_to_pages[index_name].append(paper["start_page"])
